...
export class PatientService {
    ...
    public async getRecallById(id: number, dieticianId: number) {
        ...
        if (!this.userRepository.isPatientDieticians({patientId: recall.patientId, dieticianId})) {
            throw new UnauthorizedError("You are not authorized to access this recall");
        }
        return recall
    }
